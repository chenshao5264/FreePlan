...
function wnet.PL_PHONE_LC_LOGIN_ACK:toString()
...
function wnet.CL_REG_REQ:bufferIn(data)
	...
	buf:writeStringUShort(data.strAccount or ""):writeStringUShort(data.strNickName or "")
	   ...
	   :writeChar(gender or 0):writeShort(icon or 1)
...
-------------------------------道具协议---------------------------------------
----------------------------------------------------------------------------
wnet.PL_PHONE_CL_GETUSERITEM = class("PL_PHONE_CL_GETUSERITEM", packBody)
function wnet.PL_PHONE_CL_GETUSERITEM:ctor(code, uid, pnum, mapid, syncid)
	wnet.PL_PHONE_CL_GETUSERITEM.super.ctor(self, code, uid or 0, pnum or 0, mapid or 0, syncid or 0)
	self.name = "PL_PHONE_CL_GETUSERITEM"
end
function wnet.PL_PHONE_CL_GETUSERITEM:bufferIn(userId, conIndex)
	local buf = wnet.PL_PHONE_CL_GETUSERITEM.super.bufferIn(self)
	buf:writeInt(userId):writeInt(conIndex or 0)
	return buf
end
----------------------------------------------------------------------------
----------------------------------------------------------------------------
wnet.PL_PHONE_LC_GETUSERITEM = class("PL_PHONE_LC_GETUSERITEM", packBody)
function wnet.PL_PHONE_LC_GETUSERITEM:ctor(code, uid, pnum, mapid, syncid)
	wnet.PL_PHONE_LC_GETUSERITEM.super.ctor(self, code, uid or 0, pnum or 0, mapid or 0, syncid or 0)
	self.name = "PL_PHONE_LC_GETUSERITEM"
	self.phoneUserItemInfo = {}
end
function wnet.PL_PHONE_LC_GETUSERITEM:bufferOut(buf)
	self.userId = buf:readInt()
	self.conIndex = buf:readInt()
	self.exChangePhone = buf:readInt()
	local size = buf:readShort()
	for i = 1, size do
		local itemInfo = wnet.PhoneUserItemInfo.new()
		itemInfo:bufferOut(buf)
		self.phoneUserItemInfo[#self.phoneUserItemInfo + 1] = itemInfo
	end
end
----------------------------------------------------------------------------
----------------------------------------------------------------------------
wnet.PhoneUserItemInfo = class("PhoneUserItemInfo", packBody)
function wnet.PhoneUserItemInfo:ctor(code, uid, pnum, mapid, syncid)
	wnet.PhoneUserItemInfo.super.ctor(self, code, uid or 0, pnum or 0, mapid or 0, syncid or 0)
	self.name = "PhoneUserItemInfo"
end
function wnet.PhoneUserItemInfo:bufferOut(buf)
	self.modualId = buf:readInt()
	self.modualNum = buf:readInt()
end
----------------------------------------------------------------------------
----------------------------------------------------------------------------
wnet.PL_PHONE_CL_EXCHANGEPHONE = class("PL_PHONE_CL_EXCHANGEPHONE", packBody)
function wnet.PL_PHONE_CL_EXCHANGEPHONE:ctor(code, uid, pnum, mapid, syncid)
	wnet.PL_PHONE_CL_EXCHANGEPHONE.super.ctor(self, code, uid or 0, pnum or 0, mapid or 0, syncid or 0)
	self.name = "PL_PHONE_CL_EXCHANGEPHONE"
end
function wnet.PL_PHONE_CL_EXCHANGEPHONE:bufferIn(userId, moduleId, phoneCode, exChangeMoney, dConnIndex)
	local buf = wnet.PL_PHONE_CL_EXCHANGEPHONE.super.bufferIn(self)
	buf:writeInt(userId or 0):writeInt(moduleId or 0):writeStringUShort(phoneCode or ""):writeInt(exChangeMoney or 0):writeInt(dConnIndex or 0)
	return buf
end
----------------------------------------------------------------------------
----------------------------------------------------------------------------
wnet.PL_PHONE_LC_EXCHANGEPHONE = class("PL_PHONE_LC_EXCHANGEPHONE", packBody)
function wnet.PL_PHONE_LC_EXCHANGEPHONE:ctor(code, uid, pnum, mapid, syncid)
	wnet.PL_PHONE_LC_EXCHANGEPHONE.super.ctor(self, code, uid or 0, pnum or 0, mapid or 0, syncid or 0)
	self.name = "PL_PHONE_LC_EXCHANGEPHONE"
end
function wnet.PL_PHONE_LC_EXCHANGEPHONE:bufferOut(buf)
	self.userId = buf:readInt()
	self.dConnIndex = buf:readInt()
	self.nResult = buf:readInt()
	self.moduleId = buf:readInt()
	self.totalNum = buf:readInt()
	self.exChangeNum = buf:readInt()
end
----------------------------------------------------------------------------
-------------------------------道具 止----------------------------------------
...
------------------------------------------------------------------------------------------
wnet.Sys_KickUser_Reason =
{
	SysKickUser_MoneyLimit = 0,
	SysKickUser_NoReady = 1,
	SysKickUser_GainLimit = 2,
	SysKickUser_MatchScore = 3,			--比赛场时，如果比赛积分不够，则踢出玩家
}

wnet.GC_SYS_KICKUSER_NOT = class("GC_SYS_KICKUSER_NOT", packBody)
function wnet.GC_SYS_KICKUSER_NOT:ctor(code, uid, pnum, mapid, syncid)
	wnet.GC_SYS_KICKUSER_NOT.super.ctor(self, code, uid or 0, pnum or 0, mapid or 0, syncid or 0)
	self.name = "GC_SYS_KICKUSER_NOT"
end
function wnet.GC_SYS_KICKUSER_NOT:bufferOut(buf)
	self.reason = buf:readChar()
end
